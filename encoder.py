...
parser.add_argument('--cuda', '-g', action='store_true',help='disables cuda ')
parser.add_argument(
...
image = torch.from_numpy(
    np.expand_dims(
        np.transpose(image.astype(np.float32) / 255.0, (2, 0, 1)), 0))#归一化
...
encoder_h_1 = (Variable(
    torch.zeros(batch_size, 256, height // 4, width // 4), volatile=True),#True的话可以在训练时固定底层只更新上层
               Variable(
