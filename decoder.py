...
codes = np.unpackbits(content['codes'])#例如把7变成了00000111
codes = np.reshape(codes, content['shape']).astype(np.float32) * 2 - 1#astype把数值转换成了浮点型，并利用reshape将codes的维度转换为和content的维度一样
...
decoder_h_3 = (Variable(
    torch.zeros(batch_size, 256, height // 4, width // 4), volatile=True),#“//"的意思是取整除 - 返回商的整数部分
               Variable(
